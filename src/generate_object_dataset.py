# script adopted from https://github.com/OPTML-Group/Diffusion-MU-Attack
...
def generate_object_images(k, object_id, device='cuda:0', guidance_scale = 7.5, image_size=512, ddim_steps=100, num_samples=1, from_case=0, cache_dir='./ldm_pretrained', save_path=None):
    '''
    ...
    prompt_list = object_prompts(object_list[object_id])

    
    concept = object_list[object_id]
...
if __name__=='__main__':
    ...
    object_list = ['cassette_player', 'church', 'english_springer', 'french_horn', 'garbage_truck', 'gas_pump', 'golf_ball', 'parachute', 'tench', 'chain_saw']
    parser.add_argument('--concept', help='which object to generate', type=str, required=False, default='church', choices=object_list)
    ...
    k = args.k
    object_id = object_list.index(args.concept)
    generate_object_images(k, object_id, device=device,
                    guidance_scale = guidance_scale, image_size=image_size, ddim_steps=ddim_steps,
                    num_samples=num_samples,from_case=from_case,cache_dir=cache_dir, save_path=save_path)
