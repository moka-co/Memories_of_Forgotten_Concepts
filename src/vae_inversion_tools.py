...
def get_latent_from_encoder(encoder, img, device=None):
    ...
    # Resize only if the image isn't already 512x512
...
def vae_inversion_start_from_encoder_latent(
    ...
    # Instantiate gaussian loss only once
    ...
    for step in pbar:
        ...
        loss = (
            ...
            + scale_gaussian * gaussian_nll_loss(z, target_mean, target_var) #nn.GaussianNLLLoss()(z, target_mean, target_var)
        )
        ...
        if step % 100 == 0:
            ...
            print(
                ...
                f"Gaussian NLL: {gaussian_nll_loss(z, target_mean, target_var):.3f}"
            )
            rec2 = (
